...
console.log(N.numberToText(14563400153085320909, "de"));
...
const createGame = (N, initial = false) => {
  ...
  const init = (N) =>
    range(N * N).map((i) => ({ x: state.key++, n: 1, c: randomColor() }));
  ...
  const neighbors = (p) =>
    [upper, lower, left, right].map(apply(p)).filter(onBoard);
  ...
  const check = () =>
    state.stars > 0 ||
    range(state.N * state.N).some((idx) => flood(coord(idx)).length >= 3);
  const drop = (cb) => {
    let dropped = false;
  ...
  const click = (idx, cb) => {
    ...
    if (connected.length >= 3 || state.starActive) {
      ...
      connected
        .filter((i) =>
          state.field[idx].c === "RED2" || state.starActive ? true : i != idx
        )
        .forEach(
          (n) =>
            (state.field[n] = {
    ...
    use(coord(idx), (p) =>
      use(colorAt(p), (ownColor) =>
        [
          ...
          .filter((n) =>
            use(n.f(p), (np) => onBoard(np) && colorAt(np) === ownColor)
          )
          .map((n) => ".connected-" + n.s)
          .join(" ")
      )
...
const star = (vnode) => ({
  view: (vnode) =>
    svg({ height: "25px", width: "25px", onclick: vnode.attrs.onclick }, [
...
const scoreView = (vnode) => ({
  view: (vnode) => [
    small(vScore),
    " ",
    small(round(game.quality() * 100) / 100),
  ],
});

const numberView = (vnode) => ({
  view: ({ attrs }) =>
    div.margin(small.margin(N.numberToText(attrs.number, language))),
});
...
m.mount(document.body, {
  view: (vnode) => [
    ...
    !game.lost()
      ? null
      : h3.stars({ onclick: () => (game = createGame(7)) }, "Lost! Again?"),
